...
RSpec.describe Task, type: :model do
  ...
  describe 'Validators' do
    ...
    describe 'errors' do
      ...
      it 'no company role' do
        should_not allow_value(company: { company: @company, role: nil })
          .for(:task_owner).with_message('no company role set')
      end
      it 'unkown company role' do
        should_not allow_value(company: { company: @company, role: :INVALID })
          .for(:task_owner).with_message('unknown company role')
      end
      it 'no agency role' do
        should_not allow_value(agency: { agency: @agency, role: nil })
          .for(:task_owner).with_message('no agency role set')
      end
      it 'unkown agency role' do
        should_not allow_value(agency: { agency: @agency, role: :INVALID })
          .for(:task_owner).with_message('unknown agency role')
      end
    end
    describe 'success' do
      subject { FactoryBot.build(:task) }
  ...
  describe 'Find all tasks for a owner' do
    ...
    it 'job developer user' do
      expect(Task.find_by_owner_user_open(@job_developer1))
        .to eq [@task_jd, @task_jd_today]
    end
    it 'case manager user' do
      expect(Task.find_by_owner_user_open(@case_manager1))
        .to eq [@task_cm, @task_cm_today]
    end
    it 'agency admin user' do
      expect(Task.find_by_owner_user_open(@agency_admin1)).to eq [@task_aa]
    end
    it 'company admin user' do
      expect(Task.find_by_owner_user_open(@company_admin1))
        .to eq [@task_ca, @task_ca_today]
    end
    it 'company contact user' do
    ...
    context 'where the target is a job seeker' do
      ...
      context 'when a specific task type is selected' do
        it 'return all need_job_developer' do
          all_tasks = Task.find_by_type_and_target_job_seeker_open(
            'need_job_developer',
            @jane
          )
