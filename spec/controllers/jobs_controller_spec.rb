...
RSpec.describe JobsController, type: :controller do
  before(:example) do
    ...
    @job2 = FactoryGirl.create(:job, company: @company_person.company)
  ...
  describe 'Route => edit, sad path' do
    it do
      should_not route(:get, '/jobs/1/edit')
        .to(action: :show, id: 1)
    end
  end
  ...
  describe 'DELETE #destroy sad path, for different company_person ' do
    ...
    before(:example) { delete :destroy, id: @job2.id }
  ...
  describe 'GET #update_addresses' do
    ...
    it 'returns option tags for addresses' do
      expect(response.body).to have_content(address1.street.to_s)
      expect(response.body).to have_content(address2.street.to_s)
    end
  end
  ...
  describe 'GET #apply' do
    ...
    describe 'invalid application as job developer' do
      ...
      it 'show flash[:alert]' do
        expect(flash[:alert]).to be_present.and eq 'Invalid application: '\
        'You are not the Job Developer for this job seeker'
      end
      it 'redirect to job ' do
        expect(response).to redirect_to(job_path(@job))
      end
