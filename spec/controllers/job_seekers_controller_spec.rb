...
RSpec.describe JobSeekersController, type: :controller do
  ...
  describe 'POST #create' do
    context 'valid attributes' do
      before(:each) do
        ...
        @jobseeker_hash[:address_attributes] =
          FactoryGirl.attributes_for(:address, zipcode: '54321')
        post :create, job_seeker: @jobseeker_hash
      end

      it 'sets flash message' do
        expect(flash[:notice]).to eq 'A message with a confirmation and link ' \
      ...
      describe 'confirmation email' do
        ...
        it do
          is_expected.to have_body_text(%r{/users\/confirmation\?confirmation/})
        end
    ...
    context 'valid attributes and resume file upload' do
      before(:each) do
        ...
        js_status = FactoryGirl.create(:job_seeker_status)
        @jobseeker_hash = FactoryGirl.attributes_for(
          :job_seeker,
      ...
      before(:each) do
        ...
        jobseeker1_hash =
          FactoryGirl.attributes_for(:job_seeker,
                                     year_of_birth: '198',
                                     resume: fixture_file_upload(
                                       'files/Janitor-Resume.doc'
                                     ))
                     .merge(FactoryGirl.attributes_for(:user,
                                                       first_name: 'John',
                                                       last_name: 'Smith',
    ...
    context ' updated by job seeker ' do
      ...
      context 'valid attributes without password change' do
        before(:each) do
          ...
          patch :update, id: jobseeker,
                         job_seeker: FactoryGirl.attributes_for(
                           ...
                           resume: fixture_file_upload(
                             'files/Admin-Assistant-Resume.pdf'
                           )
                         )
                           .merge(job_seeker_status_id: js_status.id)
        ...
        it 'sets the valid attributes' do
          ...
          expect(jobseeker.resumes[0].file_name).to eq \
    ...
    context " updated by job seeker's case manager " do
      ...
      context 'valid attributes' do
        it 'locates the requested @jobseeker' do
          patch :update, id: case_manager.job_seekers.first,
  ...
  describe 'GET #applied_jobs' do
    ...
    context 'JS home page view' do
      before(:each) do
        sign_in job_seeker
        xhr :get, :applied_jobs, id: job_seeker.id,
                                 application_type: 'my-applied'
      end
